...
### Выводы
  ...
  Для исследования я брал три различных времени: время `построения`, `поиска` и `общее` (которое, оказалось, не является суммой первых двух, про это дальше). Такое разделение позволяет лучше понять для чего каждый из алгоритмов больше подходит.

  - **Построение структуры:**  
    Очевидно, что наилучшее время показал алгоритм `полного перебора`, не требующий построения какой-то структуры, кроме массива, со сложностью `O(N)`, или `O(1)`, если не брать в рассчет заполнение массива данными. Худшим по части построения можно назвать алгоритм `построения карты`, со сложностью `O(N^3)`, такой алгоритм явно будет плохим решением, когда требуется проверить большое количество прямоугольников. Алгоритм `персистентного ДО` показал себя твердым среднячком со сложностью `O(Nlog(N))`.

  - **Выполнение поиска:**  
    Вот тут у `полного перебора` начинаются проблемы, его временная сложность больше таковой у "конкурентов" - `O(N^2)` и `O(log(N))`, соответственно. По графику хорошо заметно, что графики `карты` и `дерева` идут одинаково (с учетом погрешности). Можно сказать, что использовать `полный перебор` с большим количеством точек - затея не очень хорошая. Тут уже можно подвести итог о `карте` и `дереве`. Имея одинаковую сложность выполнения запроса, `карта` абсолютно проигрывает в производительности построения, так что, определенно, "дерево" является более предпочтительным алгоритмом, чем `карта`

  - **Общее время выполнения:**  
    Хорошо заметно, как алгоритм построения карты проигрывает остальным. Но на удивление, `дерево` и `полный перебор` идут достаточно близко, сложно сказать, с чем это связано, может быть с входными данными, так как проверяются квадраты со сторонами равными степеням двойки, возможно, из-за особенностей языка Java, а может и от того, что под конец проверки, который приходится аккурат на `дерево`, у меня садился ноутбук. 

  - **Итог**  
    `Худший алгоритм` - [карта](#алгоритм-на-карте).  
    `Лучшие` - [дерево](#алгоритм-на-дереве) и [перебор](#алгоритм-перебора), лучше всего себя показал все же первый.
